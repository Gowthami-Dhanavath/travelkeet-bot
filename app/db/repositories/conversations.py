...
class ConversationRepo:
    ...
    async def get_or_create(self, session_id: str, user_agent: str | None = None, ip_hash: str | None = None) -> Conversation:
        """
        MUST guarantee same session_id always returns same conversation.
        """
        result = await self.session.execute(
            select(Conversation).where(Conversation.session_id == session_id)
        )
        ...
        # IMPORTANT: ensures id is generated and visible within same session
    ...
    async def update_usage(self, conv_id: UUID, tokens_in: int, tokens_out: int, cost_cents: int) -> None:
        conv = await self.session.get(Conversation, conv_id)
        if conv is None:
            raise ValueError(f"Conversation {conv_id} not found")
