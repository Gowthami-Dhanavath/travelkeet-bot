...
class CampervanRepo:
    ...
    async def search(
        self,
        *,
        base_city: str | None = None,     # FIX: match tests
        min_capacity: int | None = None,
        max_price: int | None = None,
        features_any: list[str] | None = None,  # FIX: match tests
        limit: int = 20,
    ) -> list[Campervan]:
        """Filter active vans. None means no filter on that dimension."""

        conditions = [Campervan.is_active.is_(True)]

        # FIX: case-insensitive city match
        if base_city:
        ...
        if max_price is not None:
            conditions.append(Campervan.price_per_day <= max_price)

        # FIX: feature overlap logic
        if features_any:
